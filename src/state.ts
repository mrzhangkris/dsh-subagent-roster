...
/**
 * Blank optional string fields (e.g. a model materializing `""` for an
 * omitted parameter, issue #105 class) must not persist: they would brick
 * the team on reload. Normalize blank optionals to omitted before validation.
 */
export function normalizeBlankOptionalTaskFields<T extends object>(task: T): T {
  const cleaned = { ...task } as Record<string, unknown>
  for (const key of ['profileSeedId', 'subject', 'description', 'assignee', 'output', 'attemptId'] as const) {
 ...
 * Terminal statuses have no outgoing transitions. The `claimed` status is
 * only produced by durable records from the upstream fork; new work starts
 * at `pending` and moves to `in_progress` through `agent_teams_update_task`.
 */
...
/**
 * Return one unfinished task to the pending pool and clear its legacy
 * capability, so a stale update can no longer mutate it.
 */
export function invalidateTaskAttempt(task: TeamTask, nextAssignee?: string): void {
  task.attemptId = undefined
  task.status = 'pending'
...
export type VisualTaskState = 'open' | 'running' | 'completed' | 'failed' | 'cancelled'

/**
 * The visual state of one task: `running` while in_progress, `completed`
 * when done, `failed`/`cancelled` when terminal without success, else `open`.
 */
export function taskVisualState(status: string): VisualTaskState {
