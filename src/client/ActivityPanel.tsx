...
function taskTitle(task: ActivityTask, model: string): string {
  const extras = [model === '' ? undefined : model].filter((item): item is string => item !== undefined)
...
function memberStateLabel(
  ...
  if (owned.some((task) => task.status === 'pending')) return t('member.state.waiting')
...
function taskSummary(team: ActivityTeam, t: AgentTeamsTranslate, discarded = false): string {
  ...
  if (failed.length > 0 && running.length === 0 && ready.length === 0) {
...
function ProgressOverview({ team, t, discarded = false }: { readonly team: ActivityTeam; readonly t: AgentTeamsTranslate; readonly discarded?: boolean }) {
  ...
  const pending = discarded ? 0 : team.tasks.filter((task) => task.state === 'open').length
  ...
  const summaryTone = discarded ? 'discarded' : settled ? 'completed' : 'running'
  return (
    <section className={css.progressOverview} aria-label={t('progress.aria')} data-progress-summary>
      ...
      <span className={css.progressLegend}>
        ...
        <span data-state="open">{t('progress.pending', { count: pending })}</span>
...
/** Flat task grid with a detail box: the DAG visualization was removed with
 * the task dependency graph. Selection state is purely local. */
function TaskList({ tasks, members, t, discarded = false }: {
  ...
  const fallbackTask = tasks.find((task) => task.state === 'running')
    ?? tasks.find((task) => task.state === 'open')
    ?? tasks[0]!
  const detailTask = tasks.find((task) => task.id === selectedTaskId) ?? fallbackTask
  const detailModel = taskModelLabel(detailTask, members)
  return (
    <section className={css.dependencySection} aria-label={t('dependency.aria')} data-task-list>
      <header className={css.sectionHead}>
        <button type="button" className={css.sectionToggleTitle} onClick={() => { setSelectedTaskId((current) => current === null ? detailTask.id : null) }} aria-expanded={selectedTaskId !== null}>
          <Chevron open={selectedTaskId !== null} /> {t('dependency.parallel')}
        </button>
        <span className={css.sectionHint}>{selectedTaskId === null
          ? t('dependency.hint.parallel')
          : t('dependency.hint.pinned', { taskId: selectedTaskId })}</span>
      </header>
      <div className={css.dagViewport}>
        <div className={css.dagCanvas} data-layout="parallel">
          {tasks.map((task) => {
            ...
            return (
              <button
                ...
                data-focused={selectedTaskId === task.id}
...
function TeamSection({ team, modelDirectory, onContinuePlanning, onDiscarded, onNavigate, t, historic = false }: {
  ...
  return (
    <>
      ...
      <TaskList tasks={team.tasks} members={team.members} t={t} discarded={discarded} />
      </section>
      <Modal
        open={stopOpen}
