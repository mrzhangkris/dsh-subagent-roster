 ...
 *   of guessing; a missing `capabilities` service skips the gate (the deps
 *   are optional for isolated route testing — production wiring MUST inject
 *   it, so a skipped gate is a test-only shape, never a production one).
...
export interface RouteDeps {
  ...
  llm?: unknown
  /** Host `ctx.subagents` (mocked in tests); provides `getProvider`. */
...
export async function preflightRoute(deps: RouteDeps, args: RouteArgs): Promise<RouteResult> {
  ...
  // a backend that cannot apply a required knob must fail loudly here.
  if (deps.capabilities !== undefined) {
    const gate = checkCapabilities(deps.capabilities as CapabilitiesLike, args.transport, args.spec)
