...
> Corridor coverage note: the corridor now covers `dsh-v0.1.1-rc.1 → dsh-v0.1.3-alpha.2` (when this example was written the rc.1 → rc.2 segment had no cards; it was later filled by [v0.1.1-rc.2.md](../references/v0.1.1-rc.2.md), 3 cards, `DSH-0.1.1-R2` prefix; the 0.1.2 line ends at rc.1, and the 0.1.3-line edges rc.1→0.1.3-alpha.1 and 0.1.3-alpha.1→0.1.3-alpha.2 were added as draft cards after 2026-09-07). The technical claims in this example (client-runtime removal, `ctx.slots.inject`, `views.get('chat')?.legacy`, etc.) were first-hand sources at the time of writing; the same fleet's later zero-code ride of the 0.1.2 train up to rc.1 is recorded in the "Follow-up" section at the end; the closest existing card is DSH-0.1.2-A1-03 · 会话视图工程大幅拆分 (session-view engineering split).
